...
export default function CreateOnlineGameOffer() {
    const router = useRouter();

    const [state, setState] = useState<{ validated: boolean, formState: FormState, formError: FormError }>({
        validated: false,
        formState: {...initialState, offerCategory: 'OnlineGame', offerType: 'CasinoOffer'},
    ...
    const handleFormSubmit: MouseEventHandler<HTMLButtonElement> = (e) => {
        ...
        axios
            .post('https://one-dollar-admin.onrender.com' + '/offer/create/onlineGame', formData,
                {
                    headers: {
            ...
            .then((response) => {
                console.log('OnlineGame Offer Created');

                const dataToSend = {
                    type: 'success',
                    title: 'Action Completed',
                    message: 'OnlineGame Offer has been successfully saved',
            ...
            .catch((error) => {
                console.error('OnlineGame Offer Creation failed:', error);
    ...
    return (
        <>
            <Form title="Create Online Game Offer" validated={state.validated}>
                ...
                <InputSelect name="offerCategory" label="Offer Category" options={[
                    {value: 'OnlineGame', label: 'Online Game Offer'}]}
